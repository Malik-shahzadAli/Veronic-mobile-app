...
otp: string;
  ...
  resendOTP() {
    if (this.counter < 3) {
    this.presentAlert();
    this.http.post(url.baseurl + '/api/otp/generate',{"phoneNo" : window.localStorage.getItem('loginNo')})
      .subscribe((response) => {
  ...
  validateOTP() {
    ...
    const pNumber = window.localStorage.getItem('loginNo');
  ...
  ngOnInit() {
    ...
    console.log('NUMBER----' + this.finalObj.customer.customerData.phone);
    this.number = window.localStorage.getItem('loginNo');
    ...
    this.StartTimer();
    if ((window.localStorage.getItem('loginNo'))) {
