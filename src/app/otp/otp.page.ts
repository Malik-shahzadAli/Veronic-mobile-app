...
otp: string;
  ...
  // VALIDATE OTP
  validateOTP() {
    const first = this.otpForm.value.first;
    const secound = this.otpForm.value.sec;
    const third = this.otpForm.value.third;
    const fourth = this.otpForm.value.fourth;
    const otp = first + secound + third + fourth;
    console.log(otp);
   ...
   {"phoneNo" : this.finalObj.customer.customerData.phone, "otp" : otp})
    .subscribe((response) => {
  ...
  ngOnInit() {
    console.log(this.finalObj.customer.customerData.phone);
    this.otpForm.setValue({phoneNo:this.finalObj.customer.customerData.phone,first:'',sec:'',third:'',fourth: ''})
    this.StartTimer();
